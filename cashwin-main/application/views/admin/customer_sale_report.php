<?php
include('includes/header.php');
include('includes/sidebar.php');
?>
<style>
   .heading-border{
   border: 2px dotted black;
   text-align: center;
   padding: 5px 0px;
   color: #9B5B82;
   font-weight: 600;
   margin-bottom: 10px;
   }
   .text-center th{
   text-align:center;
   }
   .text-center td{
   text-align:center;
   }
   .points {
   background: #F46A6A;
   color: white;
   border-radius: 5px;
   }
   .points-table .table{
   display: block;
   border:0px!important;
   overflow-x: scroll;
   }
   .points-custum-table tr{
   display:flex;
   }
   .points-custum-table td {
   width: 120px;
   display: inline-block;
   font-weight:600;
   }
   .custum-responsive{
   display: block;
   /*overflow-x: scroll;*/
   }
   .custum-responsive tr{
   /*display:block;*/
   /*display:flex;*/
   }
   /*.custum-responsive th {*/
   /*width: 120px;*/
   /*display: inline-block;*/
   /*font-weight:600;*/
   /*}*/
   .text-center th{
   text-align:center;
   }
   .points-custum-table>tbody>tr:nth-of-type(even){
   margin-bottom:20px;
   }
   /*scrollbar css*/
   .points-custum-table::-webkit-scrollbar {
   width: 10px;
   height:5px;
   }
   /* Track */
   .points-custum-table::-webkit-scrollbar-track {
   background: #f1f1f1;
   }
   /* Handle */
   .points-custum-table::-webkit-scrollbar-thumb {
   background: #888;
   }
   /* Handle on hover */
   .points-custum-table::-webkit-scrollbar-thumb:hover {
   background: #555;
   }
   .custum-responsive::-webkit-scrollbar {
   width: 10px;
   height:5px;
   }
   /* Track */
   .custum-responsive::-webkit-scrollbar-track {
   background: #f1f1f1;
   }
   /* Handle */
   .custum-responsive::-webkit-scrollbar-thumb {
   background: #888;
   }
   /* Handle on hover */
   .custum-responsive::-webkit-scrollbar-thumb:hover {
   background: #555;
   }
   /*scrollbar css*/
   /*css added by 10/06*/
   .break-table{
           box-sizing: initial;
   }
.custum-responsive th {
        /*width: 8.139%;*/
        width: 72px;
  display: inline-block;
   font-weight:600;
   padding:0px!important;
   margin-bottom:15px;
   border-left:0px!important;
   border-color:black!important;
   }
   .custum-responsive .border-left-1{
    border-left: 1px solid black!important;
   }
   .break-table .digit{
       text-align:center;
       padding:8px 10px;
        border-bottom: 1px solid black;
        border-top: 1px solid black;
   }
   .break-table .point{
       text-align:center;
        padding:8px 10px;
   }
   .break-table table{
       border:0px!important;
   }
   /*css added by 10/06*/
</style>
    <div class="content-wrapper">
      <div class="pd-30">
        <h4 class="tx-gray-800 mg-b-5"> </h4>
      </div>
        <div class="container-fluid">
            <div class="card">
                <div class="card-header">      
                    <h4 class="tx-gray-800 mg-b-5">Customer Sale Report</h4> 
                </div>
                <div class="card-body">
                    <div class="pd-y-20 bd">
                        <form method="post" action="<?= base_url() ?>report/customer_sale_report/<?= $game_type ?>" id="biddingform">
                            <div class="col-md-12">
                                <div class="row">
                                    <?php $curr_date = date('Y-m-d') ?>
                                    <div class="col-md-3">
                                        <label>Game Name</label><br>
                                        <select id="matka" name="matka" class="form-control">
                                            <?php foreach ($matka as $m): ?>
                                                <option value="<?php echo $m->id ?>" <?php echo ($select_matka == $m->id) ? "selected" : "" ?>><?php echo $m->name; ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    <div class="col-md-3">
                                        <label>Game Type</label>
                                            <br>
                                        <select id="game" name="game" class="form-control">
                                             <option value="">All</option>
                                            <?php foreach ($games as $g): ?>
                                                <option value="<?php echo $g->game_id ?>" <?php echo ($select_game == $g->game_id) ? "selected" : "" ?>><?php echo $g->name; ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    <div class="col-md-3">
                                        <label>Game Session</label>
                                            <br>
                                        <select id="session" name="session" class="form-control">
                                            <?php
                                            if ($game_type == 'starline') {
                                                ?>
                                                 <option value="open" <?php echo ($select_session == "open") ? "selected" : "" ?>>Open</option>
                                            <?php } else { ?>
                                                <option value="open" <?php echo ($select_session == "open") ? "selected" : "" ?>>Open</option>
                                                <option value="close" <?php echo ($select_session == "close") ? "selected" : "" ?>>Close</option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                    <div class="col-md-3">
                                        <label>Date</label>
                                        <br>
                                        <input type="date" id="select_date" required name="select_date" <?php echo ($select_date != "") ? 'value=' . $select_date : 'value=' . $curr_date ?> class="form-control" max="<?= $curr_date; ?>">
                                    </div>

                                    <div class="col-md-12 text-center">
                                        <input type="button" class="btn btn-danger mt-2" name="calcel" value="Cancel" onClick="window.location.reload();">
                                        <input type="submit"  class="btn btn-secondary mt-2" id="sbmt_btn" name="sbmt_btn" value="Submit">
                                    </div>
                                </div>
                            </div> 
                        </form> 
                    </div>
                    <br>
                    
                    <?php
                    $sd_arr = array();
                    foreach ($single_digit as $sd) {
                        array_push($sd_arr, $sd->digits);
                    }
                    // print_r($sd_arr);
                    
                    $jd_arr = array();
                    foreach ($jodi_digit as $jd) {
                        array_push($jd_arr, $jd->digits);
                    }
                    // print_r($jd_arr);
                    $sp_arr = array();
                    foreach ($single_pana as $sp) {
                        array_push($sp_arr, $sp->digits);
                    }
                    // print_r($sp_arr);
                    $dp_arr = array();
                    foreach ($double_pana as $dp) {
                        array_push($dp_arr, $dp->digits);
                    }
                    // print_r($dp_arr);
                    $tp_arr = array();
                    foreach ($triple_pana as $tp) {
                        array_push($tp_arr, $tp->digits);
                    }
                    // print_r($tp_arr);
                    ?>
                    
<!--pana logic start-->
<?php
$str = "";
$single_digit_array = $left_digit_array = $right_digit_array = range(0, 9);
$single_pana_array = $single_pana_array;
$double_pana_array = $double_pana_array;
$triple_pana_array = $triple_pana_array;
$jodi_digit_array = $jodi_digits_array = range(00, 99);
?> 
<!--pana logic end-->

                    <?php
                    foreach ($selectedGames as $game) {

                        $gameDataByID = $gameData[$game->game_id];
                        //foreach($gameData[$game_id->game_id] as $gameDataByID){ ?>
                            
                                    <div class="break-table d-none">
                                       <table class="table table-bordered custum-responsive">
                                          <thead>
                                             <tr>
                                                <th class="border-left-1">
                                                    <div class="digit"> Digit</div>
                                                    <div class="point">Point</div>
                                                </th>
                                                <!--digit point section-->

                                                <?php
                                                $arrayData = $game->game_name . '_array';
                                                $data_digits = $$arrayData;
                                                if ($game->game_id == 12 || $game->game_id == 13)
                                                    $data_digits = $gameData[$game->game_id];

                                                $sd_arr = array();
                                                foreach ($gameData[$game->game_id] as $sd) {
                                                    // $sd_digits = $sda;
                                                    // if($game->game_id==12  || $game->game_id==13)
                                                    //     $sd_digits = $sd->digits;
                                                    array_push($sd_arr, $sd->digits);
                                                }

                                                foreach ($data_digits as $sda) {
                                                    $sda_digits = $sda;
                                                    $sdrr_points = 0;
                                                    if ($game->game_id == 12 || $game->game_id == 13)
                                                        $sda_digits = $sda->digits;
                                                    if (in_array($sda_digits, $sd_arr)) {
                                                        foreach ($gameData[$game->game_id] as $sdrr) {
                                                            if ($sda_digits == $sdrr->digits)
                                                                $sdrr_points = $sdrr->points;
                                                        }
                                                    }
                                                    ?>
                                                        <!--<th scope="col"><?= $sda_digits; ?></th>-->
                                                        <!--<th scope="col"><div class="points"><?= $sdrr_points; ?></div></th>-->
                                                
                                                        <th>
                                                            <div class="digit"><?= $sda_digits; ?></div>
                                                            <div class="point"><div class="points"><?= $sdrr_points; ?></div></div>
                                                        </th>
                                                <?php } ?>

                                             </tr>
                                          </thead>
                                     
                                       </table>

                                    </div>
                        <!--new jodi degit design-->


                        <h4 class="heading-border"><?= $game->name ?></h4>
                        <!--<div class="table-responsive">-->
                        <!--<table class="table table-bordered text-center">-->
                        <div class="<?php echo ($game->name == 'Single Digit' || $game->name == 'Triple Panna') ? "table-responsive" : 'break-table' ?>">
                         <table class="<?php echo ($game->name == 'Single Digit' || $game->name == 'Triple Panna') ? "table table-bordered text-center" : 'table table-bordered custum-responsive' ?>">
                           <!-- <div class="break-table">-->
                           <!--<table class="table table-bordered custum-responsive">-->
                        <!--<table class="table table-bordered text-center">-->
                         <!--static-->
                            
                         <!--static-->
                          <thead class="d-none">
                            <tr>
                                <th>Digit</th>
                                <?php
                                $arrayData = $game->game_name . '_array';
                                $data_digits = $$arrayData;
                                if ($game->game_id == 12 || $game->game_id == 13)
                                    $data_digits = $gameData[$game->game_id];

                                foreach ($data_digits as $sda) { ?>
                                    <?php
                                    if ($game->game_id == 12 || $game->game_id == 13) {
                                        //$digits= $sda->digits;
                                        ?>
                                           <th scope="col"><?= $sda->digits; ?></th>
                                       <?php } else { ?>
                                            <th scope="col"><?= $sda; ?></th>
                                       <?php } ?>
                                <?php } ?>
                            </tr>
                          </thead>
                          <tbody>
                             <tr>
                                <th class="border-left-1">
                                    <div class="digit"> Digit</div>
                                    <div class="point">Point</div>
                                </th>
                                <!--digit point section-->

                                <?php
                                $arrayData = $game->game_name . '_array';
                                $data_digits = $$arrayData;
                                if ($game->game_id == 12 || $game->game_id == 13)
                                    $data_digits = $gameData[$game->game_id];

                                $sd_arr = array();
                                foreach ($gameData[$game->game_id] as $sd) {
                                    // $sd_digits = $sda;
                                    // if($game->game_id==12  || $game->game_id==13)
                                    //     $sd_digits = $sd->digits;
                                    array_push($sd_arr, $sd->digits);
                                }

                                foreach ($data_digits as $sda) {
                                    $sda_digits = $sda;
                                    $sdrr_points = 0;
                                    if ($game->game_id == 12 || $game->game_id == 13)
                                        $sda_digits = $sda->digits;
                                    if (in_array($sda_digits, $sd_arr)) {
                                        foreach ($gameData[$game->game_id] as $sdrr) {
                                            if ($sda_digits == $sdrr->digits)
                                                $sdrr_points = $sdrr->points;
                                        }
                                    }
                                    ?>
                                        <!--<th scope="col"><?= $sda_digits; ?></th>-->
                                        <!--<th scope="col"><div class="points"><?= $sdrr_points; ?></div></th>-->
                                
                                        <th>
                                            <div class="digit"><?= $sda_digits; ?></div>
                                            <div class="point"><div class="points"><?= $sdrr_points; ?></div></div>
                                        </th>
                                <?php } ?>

                             </tr>
                          </tbody>
                        </table>
                        </div>
                    <?php } //} ?>                    

                  
                    
                    <!--new jodi degit design-->
                        <?php if (isset($jodi_digit) && $jodi_digit != "") { ?>
                                    <h4 class="heading-border d-none" >Jodi Digit</h4>
                                    <div class="break-table" >
                                       <table class="table table-bordered custum-responsive d-none">
                                          <thead>
                                             <tr>
                                                <th class="border-left-1">
                                                     <div class="digit"> Digit</div>
                                                    <div class="point">Point</div>
                                                </th>
                                                <!--digit point section-->
                                                <?php
                                                $jodi_digit_array = array();
                                                for ($i = 0; $i < 10; $i++) {
                                                    for ($j = 0; $j < 10; $j++) {
                                                        $jodi_digit_formation = $i . $j;
                                                        array_push($jodi_digit_array, $jodi_digit_formation);
                                                        ?>
                                                        <th>
                                                            <div class="digit"><?= $jodi_digit_formation; ?></div>
                                                            <div class="point">
                                                                <!--point button-->
                                                                  <div class="points">
                                                                    <?php
                                                                    $digits = array_column($jodi_digit, 'digits');
                                                                    $points = array_column($jodi_digit, 'points');
                                                                    $PointsArray = array_combine($digits, $points);
                                                                    if (in_array($jodi_digit_array[abs($i . $j)], $jd_arr)) {
                                                                        echo $PointsArray[$i . $j];
                                                                    } else {
                                                                        echo "00";
                                                                    }


                                                                    ?>      
                                                         
                                                                      </div>
                                                                             <!--point button-->
                                                                        </div>
                                                     
                                                                    <?php


                                                    }
                                                } ?>
                                                                  </th>
                                                   <!--digit point section-->
                                             </tr>
                                          </thead>
                                     
                                       </table>
                                    
                                       <table class="table table-bordered custum-responsive text-center d-none">
                                          <thead>
                                             <tr>
                                                <th>Digit</th>
                                                <?php
                                                $jodi_digit_array = array();
                                                for ($i = 0; $i < 10; $i++) {
                                                    for ($j = 0; $j < 10; $j++) {
                                                        $jodi_digit_formation = $i . $j;
                                                        array_push($jodi_digit_array, $jodi_digit_formation);
                                                        ?>
                                                        <th scope="col"><?= $jodi_digit_formation; ?></th>
                                                        <?php
                                                    }
                                                } ?>
                                             </tr>
                                          </thead>
                                          <tbody>
                                             <th>Point</th>
                                             <?php foreach ($jodi_digit_array as $jda) {
                                                 if (in_array($jda, $jd_arr)) {

                                                     foreach ($jodi_digit as $jdrr) {

                                                         if ($jda == $jdrr->digits) { ?>
                                                             <th scope="col">
                                                                <div class="points"><?= $jdrr->points; ?></div>
                                                             </th>
                                                         <?php }
                                                     }
                                                 } else { ?>
                                                     <th scope="col">
                                                        <div class="points"><?= 0; ?></div>
                                                     </th>
                                                 <?php } ?>
                                             <?php } ?>
                                          </tbody>
                                       </table>
                                    </div>
                                <?php } ?>
                    <!--new jodi degit design-->
                    
                    <!--new single pana design-->
                        <?php if (isset($single_pana) && $single_pana != "") { ?>
                                <h4 class="heading-border d-none">Single Pana</h4>
                                        <div class="break-table" >
                                           <table class="table table-bordered custum-responsive d-none">
                                              <thead>
                                                 <tr>
                                                    <th class="border-left-1">
                                                         <div class="digit"> Digit</div>
                                                        <div class="point">Point</div>
                                                    </th>
                                               
                                                
                                                   <?php

                                                   foreach ($single_pana_array as $spa) {
                                                       // $digit = implode("",$spa)
                                                       $digit = $spa;
                                                       ?>
                                                    <th>
                                                        <div class="digit"><?= $digit; ?></div>
                                                        <div class="point">
                                                            <!--point button-->
                                                              <div class="points">
                                                       
                                                      
                                                                <?php


                                                                if (in_array($digit, $sp_arr)) {

                                                                    foreach ($single_pana as $sprr) {

                                                                        if ($digit == $sprr->digits) { ?>
                                    
                                                                        <?= $sprr->points; ?>
                                                                    <?php }
                                                                    }
                                                                } else { ?>
                                                                <?= 0; ?>
                                                            <?php } ?>

                                                         
                                                                  </div>
                                                                         <!--point button-->
                                                                    </div>
                                                     
                                                                <?php



                                                   } ?>
                                                                  </th>
                                                   <!--digit point section-->
                                        
                                                
                                                
                                                     <!--digit point section-->
                                                 </tr>
                                              </thead>
                                         
                                           </table>
                                        
                                           <table class="table table-bordered custum-responsive text-center d-none">
                                              <thead>
                                                 <tr>
                                                    <th>Digit</th>
                                                    <?php
                                                    $jodi_digit_array = array();
                                                    for ($i = 0; $i < 10; $i++) {
                                                        for ($j = 0; $j < 10; $j++) {
                                                            $jodi_digit_formation = $i . $j;
                                                            array_push($jodi_digit_array, $jodi_digit_formation);
                                                            ?>
                                                            <th scope="col"><?= $jodi_digit_formation; ?></th>
                                                            <?php
                                                        }
                                                    } ?>
                                                 </tr>
                                              </thead>
                                              <tbody>
                                                 <th>Point</th>
                                                 <?php foreach ($jodi_digit_array as $jda) {
                                                     if (in_array($jda, $jd_arr)) {

                                                         foreach ($jodi_digit as $jdrr) {

                                                             if ($jda == $jdrr->digits) { ?>
                                                                 <th scope="col">
                                                                    <div class="points"><?= $jdrr->points; ?></div>
                                                                 </th>
                                                             <?php }
                                                         }
                                                     } else { ?>
                                                         <th scope="col">
                                                            <div class="points"><?= 0; ?></div>
                                                         </th>
                                                     <?php } ?>
                                                 <?php } ?>
                                              </tbody>
                                           </table>
                                        </div>
                        <?php } ?>
                    <!--new single pana design-->
                    
                    <!--new double pana design-->
                        <?php if (isset($double_pana) && $double_pana != "") { ?>
                                <h4 class="heading-border d-none">Double Pana</h4>
                                        <div class="break-table d-none" >
                                           <table class="table table-bordered custum-responsive d-none">
                                              <thead>
                                                 <tr>
                                                    <th class="border-left-1">
                                                         <div class="digit"> Digit</div>
                                                        <div class="point">Point</div>
                                                    </th>
                                                
                                               
                                                          <?php

                                                          foreach ($double_pana_array as $dpa) {
                                                              //   $digit = implode("",$dpa);
                                                              $digit = $dpa;

                                                              ?>
                                                                <th>
                                                            <div class="digit"><?= $digit ?></div>
                                                            <div class="point">
                                                                <!--point button-->
                                                               <div class="points">
                                                       
                                                                <?php


                                                                if (in_array($digit, $dp_arr)) {

                                                                    foreach ($double_pana as $dprr) {

                                                                        if ($digit == $dprr->digits) { ?>
                                    
                                                                        <?= $dprr->points; ?>
                                                                    <?php }
                                                                    }
                                                                } else { ?>
                                                                <?= 0; ?>
                                                            <?php } ?>

                                                               </div>
                                                                 <!--point button-->
                                                            </div>
                                                        <?php } ?>
                                                    </th>
                                                
                                                     <!--digit point section-->
                                                 </tr>
                                              </thead>
                                         
                                           </table>
                                        
                                           <table class="table table-bordered custum-responsive text-center d-none">
                                              <thead>
                                                 <tr>
                                                    <th>Digit</th>
                                                    <?php
                                                    $jodi_digit_array = array();
                                                    for ($i = 0; $i < 10; $i++) {
                                                        for ($j = 0; $j < 10; $j++) {
                                                            $jodi_digit_formation = $i . $j;
                                                            array_push($jodi_digit_array, $jodi_digit_formation);
                                                            ?>
                                                            <th scope="col"><?= $jodi_digit_formation; ?></th>
                                                            <?php
                                                        }
                                                    } ?>
                                                 </tr>
                                              </thead>
                                              <tbody>
                                                 <th>Point</th>
                                                 <?php foreach ($jodi_digit_array as $jda) {
                                                     if (in_array($jda, $jd_arr)) {

                                                         foreach ($jodi_digit as $jdrr) {

                                                             if ($jda == $jdrr->digits) { ?>
                                                                 <th scope="col">
                                                                    <div class="points"><?= $jdrr->points; ?></div>
                                                                 </th>
                                                             <?php }
                                                         }
                                                     } else { ?>
                                                         <th scope="col">
                                                            <div class="points"><?= 0; ?></div>
                                                         </th>
                                                     <?php } ?>
                                                 <?php } ?>
                                              </tbody>
                                           </table>
                                        </div>
                        <?php } ?>
                    <!--new double pana design-->
                    <!--new Tripal pana design-->
                        <?php if (isset($triple_pana) && $triple_pana != "") { ?>
                                <h4 class="heading-border d-none">Triple Pana</h4>
                                        <div class="break-table d-none" >
                                           <table class="table table-bordered custum-responsive d-none">
                                              <thead>
                                                 <tr>
                                                    <th class="border-left-1">
                                                         <div class="digit"> Digit</div>
                                                        <div class="point">Point</div>
                                                    </th>
                                               
                                               
                                                       <?php foreach ($triple_pana_array as $tpa) {
                                                           //$digit = implode("",$tpa)
                                                           $digit = $tpa
                                                               ?>
                                              
                                                        <th>
                                                            <div class="digit"><?= $digit ?></div>
                                                            <div class="point">
                                                                <!--point button-->
                                                               <div class="points">
                                                           
                                                           
                                                                <?php


                                                                if (in_array($digit, $tp_arr)) {

                                                                    foreach ($triple_pana as $tprr) {

                                                                        if ($digit == $tprr > digits) { ?>
                                    
                                                                        <?= $tprr->points; ?>
                                                                    <?php }
                                                                    }
                                                                } else { ?>
                                                                <?= 0; ?>
                                                            <?php } ?>

                                                               </div>
                                                                 <!--point button-->
                                                            </div>
                                                        <?php } ?>
                                                    </th>
                                               
                                                     <!--digit point section-->
                                                 </tr>
                                              </thead>
                                         
                                           </table>
                                        
                                           <table class="table table-bordered custum-responsive text-center d-none">
                                              <thead>
                                                 <tr>
                                                    <th>Digit</th>
                                                    <?php
                                                    foreach ($jodi_digit_array as $jda) {
                                                        ?>
                                                        <th scope="col"><?= $jda; ?></th>
                                                    <?php } ?>
                                                 </tr>
                                              </thead>
                                              <tbody>
                                                 <th>Point</th>
                                                 <?php foreach ($jodi_digit_array as $jda) {
                                                     if (in_array($jda, $jd_arr)) {

                                                         foreach ($jodi_digit as $jdrr) {

                                                             if ($jda == $jdrr->digits) { ?>
                                                                 <th scope="col">
                                                                    <div class="points"><?= $jdrr->points; ?></div>
                                                                 </th>
                                                             <?php }
                                                         }
                                                     } else { ?>
                                                         <th scope="col">
                                                            <div class="points"><?= 0; ?></div>
                                                         </th>
                                                     <?php } ?>
                                                 <?php } ?>
                                              </tbody>
                                           </table>
                                        </div>
                        <?php } ?>
                    <!--new Tripal pana design-->
                    
                    
                    
    
                </div>
            </div>
        </div>
    </div>

<script>
//     $(document).ready(function(){
//     var date = $('#select_date').val();
//     $('#datatable').DataTable({
//             "processing": true,
//             "serverSide": true,
//             "ordering":false,
//              "searching": true,
//              'serverMethod': 'post',
//         "ajax": {
//             "url": "<?php echo base_url('admin/allbiddingreportdata/'); ?>",
//             "type": "POST",
//             "data": {"date":date},
//         },
          
//          "columns": [
//              {data:"#"},
//              {data: "date"}, 
//              {data: "bidding_digit"}, 
//              {data: "bidding_points"},
//              {data: "winning_points"},
             
             
//              ],
//               "aLengthMenu": [
//                 [50, 100,],
//                 [50, 100,]
//             ],
//             "iDisplayLength": 50
//     });
//   });
  
  
//   //get filtered data


//     function yourJsFunction(){
//     var date = $('#select_date').val();
//     var matka = $('#matka').val();
//     var game = $('#game').val();
//     var session = $('#session').val();
//     $('#datatable').html('');
//     // alert(date);alert(matka);alert(game);alert(session);
//     $('#datatable').DataTable({
//              "destroy": true,
//             "processing": true,
//             "serverSide": true,
//             "ordering":false,
//              "searching": true,
//              'serverMethod': 'post',
//         "ajax": {
//             "url": "<?php echo base_url('admin/allbiddingreportdata/'); ?>",
//             "type": "POST",
//             "data": {"sbmt_btn":1,"date":date,"matka":matka,"game":game,"session":session},
//         },
          
//          "columns": [
//              {data:"#"},
//              {data: "date"}, 
//              {data: "bidding_digit"}, 
//              {data: "bidding_points"},
//              {data: "winning_points"},
             
             
//              ],
//               "aLengthMenu": [
//                 [50, 100,],
//                 [50, 100,]
//             ],
//             "iDisplayLength": 50
//     });
//   }
  
  
</script>
<?php include('includes/footer.php'); ?>